  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 mt-6">
      <div className="p-4 border-b border-slate-200 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-800">Cảnh báo hạn sử dụng</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Mã lô</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Sản phẩm</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase">Số lượng</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Hạn sử dụng</th>
            ...
            {allBatches.map((batch) => {
              ...
              return (
                <tr key={batch.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 text-sm font-mono text-slate-900">{batch.batch_number}</td>
                  <td className="px-4 py-3 text-sm text-slate-900">{product?.name || "N/A"}</td>
                  <td className="px-4 py-3 text-sm text-slate-900 text-right">{batch.quantity}</td>
                  <td className="px-4 py-3 text-sm text-slate-900">{new Date(batch.expiry_date).toLocaleDateString("vi-VN")}</td>
