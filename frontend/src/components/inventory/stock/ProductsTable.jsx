          ...
          <tbody className="divide-y divide-gray-200">
            {products.map((product) => (
              <tr key={product.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-sm text-slate-900 text-right">{product.retail_price?.toLocaleString()}</td>
                <td className="px-4 py-3 text-sm text-slate-900 text-right">{product.purchase_price?.toLocaleString()}</td>
