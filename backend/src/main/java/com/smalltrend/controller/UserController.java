...
public class UserController {
    ...
    public ResponseEntity<?> createEmployeeAccount(@Valid @RequestBody RegisterRequest request) {
        try {
            ...
            log.error("Employee account creation failed: {}", e.getMessage(), e);
            return ResponseEntity.badRequest()
    ...
    public ResponseEntity<?> getAllUsers(
        ...
        try {
            ...
            log.error("Failed to get users list: {}", e.getMessage(), e);
            return ResponseEntity.badRequest()
                    .body(new MessageResponse("Không thể lấy danh sách người dùng"));
        }
    }
    ...
    public ResponseEntity<?> updateUserStatus(@PathVariable Integer id, @Valid @RequestBody UserStatusRequest request) {
        try {
            ...
            log.error("Failed to update user status for ID {}: {}", id, e.getMessage());
            return ResponseEntity.badRequest()
                    .body(new MessageResponse(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> searchUsers(
        ...
        try {
            ...
            log.error("Failed to search users with query '{}': {}", query, e.getMessage(), e);
            return ResponseEntity.badRequest()
                    .body(new MessageResponse("Không thể tìm kiếm người dùng"));
        }
    }
}
