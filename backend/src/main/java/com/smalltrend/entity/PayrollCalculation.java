...
public class PayrollCalculation {
    ...
    private void calculateTotals() {
        ...
        if (basePay != null) totalIncome = totalIncome.add(basePay);
        if (overtimePay != null) totalIncome = totalIncome.add(overtimePay);
        if (nightShiftBonus != null) totalIncome = totalIncome.add(nightShiftBonus);
        if (weekendBonus != null) totalIncome = totalIncome.add(weekendBonus);
        if (holidayBonus != null) totalIncome = totalIncome.add(holidayBonus);
        if (allowances != null) totalIncome = totalIncome.add(allowances);
        if (commissionAmount != null) totalIncome = totalIncome.add(commissionAmount);
        if (bonusAmount != null) totalIncome = totalIncome.add(bonusAmount);
        
        grossPay = totalIncome;
        
        // Tính tổng khấu trừ
        BigDecimal deductions = BigDecimal.ZERO;
        if (latePenalty != null) deductions = deductions.add(latePenalty);
        if (absentPenalty != null) deductions = deductions.add(absentPenalty);
        if (socialInsurance != null) deductions = deductions.add(socialInsurance);
        if (healthInsurance != null) deductions = deductions.add(healthInsurance);
        if (unemploymentInsurance != null) deductions = deductions.add(unemploymentInsurance);
        if (personalIncomeTax != null) deductions = deductions.add(personalIncomeTax);
