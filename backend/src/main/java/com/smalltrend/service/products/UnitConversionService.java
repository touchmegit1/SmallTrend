...
public class UnitConversionService {
        ...
        public UnitConversionResponse addConversion(Integer variantId, UnitConversionRequest request) {
                ...
                if (productId != null && unitId != null) {
                        List<UnitConversion> conversionsToThisUnit = unitConversionRepository
                                        .findByProductIdAndToUnitId(productId, unitId);
                        boolean isConversionDerivedVariant = conversionsToThisUnit.stream()
                                        ...
                                        .anyMatch(conversion -> hasSameAttributes(conversion.getVariant(),
                                                        baseVariant));

                        if (isConversionDerivedVariant) {
                                throw new RuntimeException(
                                                "Chỉ biến thể đơn vị gốc mới được phép thêm quy đổi đơn vị.");
                        }
                }
                ...
                if (baseStocks != null && !baseStocks.isEmpty()
                        ...
                        for (InventoryStock baseStock : baseStocks) {
                                ...
                                ProductBatch newBatch = ProductBatch.builder()
                                                ...
                                                .mfgDate(sourceBatch != null ? sourceBatch.getMfgDate()
                                                                : LocalDate.now())
                                                .expiryDate(sourceBatch != null ? sourceBatch.getExpiryDate()
        ...
        public void deleteConversion(Integer conversionId) {
                ...
                List<ProductVariant> autoVariants = productVariantRepository.findByProductIdAndUnitId(productId,
                                toUnitId);
        ...
        private boolean hasSameAttributes(ProductVariant left, ProductVariant right) {
                Map<String, String> leftAttrs = left != null && left.getAttributes() != null ? left.getAttributes()
                                : Collections.emptyMap();
                Map<String, String> rightAttrs = right != null && right.getAttributes() != null ? right.getAttributes()
                                : Collections.emptyMap();
